...
function MomoReturnContent() {
  ...
  useEffect(() => {
    ...
    // orderId có dạng orderCode_timestamp
    ...
    setResult({
      ...
      transId,
      resultCode,
      message: isSuccess ? "Giao dịch qua MoMo thành công!" : message || "Giao dịch không thành công",
    });
    setIsLoading(false);
  }, [searchParams]);
  ...
  if (isLoading || !result) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center space-y-3 font-mono text-xs">
        ...
        <p className="text-zinc-600">Đang đối soát kết quả từ ví MoMo...</p>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-zinc-50 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md mx-auto bg-white border border-zinc-200 shadow-xl p-6 md:p-8 space-y-6">
        ...
        <div className="text-center space-y-3">
          ...
          <div className="space-y-1">
            <h1 className="text-lg font-black uppercase tracking-tight text-zinc-900 font-sans">
              {result.isSuccess ? "Thanh Toán MoMo Thành Công!" : "Thanh Toán MoMo Thất Bại"}
            </h1>
        ...
        <div className="bg-zinc-50 border border-zinc-200 p-4 space-y-2.5 text-xs font-mono">
          ...
          <div className="flex justify-between pb-2 border-b border-zinc-200">
            <span className="text-zinc-500 uppercase">Phương thức:</span>
            <span className="font-bold text-pink-600">Ví MoMo / ATM Napas</span>
          </div>

          {result.transId && (
            <div className="flex justify-between pb-2 border-b border-zinc-200">
              <span className="text-zinc-500 uppercase">Mã giao dịch MoMo:</span>
              <span className="text-zinc-700">{result.transId}</span>
            </div>
          )}
        ...
        <div className="flex items-center justify-center gap-1.5 text-[11px] text-zinc-400 font-mono pt-2 border-t border-zinc-100">
          ...
          <span>Giao dịch bảo mật chuẩn quốc tế qua cổng MoMo</span>
        </div>
      </div>
    </div>
  );
